...
test("crea un contexto completo y legible", () => {
  const result = formatChatGPTContext({
    ...
    variant: { code: "B", name: "Microdiagnóstico" },
  });
...
test("omite campos vacíos y no imprime placeholders", () => {
  const result = formatChatGPTContext({
    title: "Proyecto mínimo",
    contact_name: " ",
    technologies: [],
    experiment: { name: "Experimento" },
    variant: { code: "A", name: "Pregunta" },
  });
  assert.match(result, /EXPERIMENTO\nExperimento/);
  assert.match(result, /VARIANTE\nA — Pregunta/);
  assert.doesNotMatch(result, /Contacto:|Tecnologías:|undefined|null/);
});
